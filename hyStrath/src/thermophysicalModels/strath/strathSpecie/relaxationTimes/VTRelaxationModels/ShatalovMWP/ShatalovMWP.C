...
namespace Foam
{
    ...
    {
        defineTypeNameAndDebug(ShatalovMWP, 0);
...
Foam::VTRelaxationModels::ShatalovMWP::ShatalovMWP
(
    const word& name1,
    const word& name2,
    const label& lname1,
...
{   
    ...
    Tlow_ = readScalar(dict2T.subDict("ShatalovCoefficients").lookup("Tlow"));
    Thigh_ = readScalar(dict2T.subDict("ShatalovCoefficients").lookup("Thigh"));

    ShatON_ = false;
    
    if (species1_ == species2_ and dict2T.subDict("ShatalovCoefficients").isDict(name1+"_"+name2))
    {
        ShatON_ = true;
    } 
    ...
    if(ShatON_)
    {
        word subDictNameShat = name1+"_"+name2;

        FixedList<scalar,4> defaultList(0.0);
	empir_ = dict2T.subDict("ShatalovCoefficients").lookupOrDefault<FixedList<scalar,4>>(subDictNameShat, defaultList);
    }
...
Foam::VTRelaxationModels::ShatalovMWP::tauVT() const
{
    ...
    {
	if(ShatON_ and this->Tt_[celli] > Tlow_)
	{
	    scalar x = pow(this->Tt_[celli], -1.0/3.0);
	    scalar logPTau = empir_[0]+empir_[1]*x+empir_[2]*pow(x,2)+empir_[3]*pow(x,3);
	    tauVT[celli] = 1.01325e5 / this->p_[celli] * pow(10, logPTau);
	}
	else
	{
            tauVT[celli] =
	...
	forAll(pTt, facei)
	{
	    if(ShatON_ and pTt[facei] > Tlow_)
	    {
		scalar x = pow(pTt[facei], -1.0/3.0);
		scalar logPTau = empir_[0]+empir_[1]*x+empir_[2]*pow(x,2)+empir_[3]*pow(x,3);
		ptauVT[facei] = 1.01325e5 / pp[facei] * pow(10, logPTau);
	    }
	    else
	    {
		ptauVT[facei] =
...
Foam::tmp<Foam::scalarField> Foam::VTRelaxationModels::ShatalovMWP::tauVT
(
    const label patchi,
    const scalarField& p,
...
) const
{
    ...
    forAll(Tt, facei)
    {
	if(ShatON_ and Tt[facei] > Tlow_)
	{
	    scalar x = pow(Tt[facei], -1.0/3.0);
	    scalar logPTau = empir_[0]+empir_[1]*x+empir_[2]*pow(x,2)+empir_[3]*pow(x,3);
	    tauVT[facei] = 1.01325e5 / p[facei] * pow(10, logPTau);
	}
	else
	{
	    tauVT[facei] =
