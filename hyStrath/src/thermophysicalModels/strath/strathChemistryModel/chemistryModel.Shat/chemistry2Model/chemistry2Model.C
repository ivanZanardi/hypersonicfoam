    ...
    )
{
    ...
    if(this->CVModel_ == "ParkTTv" or this->CVModel_ == "ShatalovParkTTv")
    {
        FixedList<scalar, 2> initList(0.0);
	initList[1] = readScalar(this->subDict("chemistryVibrationCoupling").subDict("ParkTTvCoeffs")
    ...
    else if(this->CVModel_ == "CVDV" or this->CVModel_ == "ShatalovCVDV")
    {
        reciprocalUFactor_ = 1.0/readScalar(this->subDict("chemistryVibrationCoupling").subDict("CVDVCoeffs").lookup("reciprocalU"));
    ...
    if(this->CVModel_ == "ShatalovParkTTv" or this->CVModel_ == "ShatalovCVDV")
    {
	a_ = readScalar(this->subDict("chemistryVibrationCoupling").subDict("ShatalovCoeffs").subDict("alpha").lookup("a")); 
	b_ = readScalar(this->subDict("chemistryVibrationCoupling").subDict("ShatalovCoeffs").subDict("alpha").lookup("b")); 
	c_ = readScalar(this->subDict("chemistryVibrationCoupling").subDict("ShatalovCoeffs").subDict("alpha").lookup("c")); 
	d_ = readScalar(this->subDict("chemistryVibrationCoupling").subDict("ShatalovCoeffs").subDict("alpha").lookup("d"));

	preferentialFactorShatalov_ = readScalar(this->subDict("chemistryVibrationCoupling")
					.subDict("ShatalovCoeffs").lookup("constantFactor")); 
    }

    Tlow_ = readScalar(this->subDict("chemistryVibrationCoupling").subDict("ShatalovCoeffs").lookup("Tlow"));
    Thigh_ = readScalar(this->subDict("chemistryVibrationCoupling").subDict("ShatalovCoeffs").lookup("Thigh"));

    // END NEW VINCENT-ZANARDI ************************************************
...
) const
{
    ...
    if(this->CVModel_ == "ParkTTv" or this->CVModel_ == "ShatalovParkTTv")
    {
        ...
        switch(controllingTemperature)
        {
            ...
            case dissociation:
                ...
                colTr = T; 
                break;
            case oxygenDissociationHighT:
		colTf = T;
                colTr = T; 
                break;
            case oxygenDissociationLowT:
		colTf = T;
                colTr = T;
                break;
            case exchange:
	...
	{
	    if(controllingTemperature != oxygenDissociationHighT and
	       controllingTemperature != oxygenDissociationLowT)
	    {
	    ...
	    else if((controllingTemperature == oxygenDissociationHighT and T > Tlow_) or
	       (controllingTemperature == oxygenDissociationLowT and T <= Tlow_))
	    {
                colTf = pow(T, this->exponentPark_)
                    *pow(spTv[R.lhs()[0].index], 1.0-this->exponentPark_);
                colTr = T;
	...
	else if(this->CVModel_ == "ShatalovParkTTv")
	{
	    if(controllingTemperature != oxygenDissociationHighT and
	       controllingTemperature != oxygenDissociationLowT)
	    {
	    ...
	    else if(T < Thigh_ and spTv[R.lhs()[0].index] < Thigh_)
	    {
		if((controllingTemperature == oxygenDissociationHighT and T > Tlow_) or
		   (controllingTemperature == oxygenDissociationLowT and T <= Tlow_) or
		   (controllingTemperature == dissociation))
		{
		    scalar alpha = a_+b_*T+c_*pow(T, 2.0)+d_*pow(T, 3.0);
		    scalar Z = exp(-alpha*(1.0-spTv[R.lhs()[0].index]/T));

		    if(this->modifiedTemperature())
		    {
			kf = R.kf(p, modTemperature(T), c2)*Z;
			kr = R.kr(kf, p, modTemperature(T), c2);
		    }
		    else
		    {
			kf = R.kf(p, T, c2)*Z;
	    ...
	    {/*
		WarningInFunction
		    << "Temperatures out of bounds for Shatalov's "
		    << "C-V model. Passing to Park's one for dissociation."
		    << endl;
	     */
	...
	else if(this->CVModel_ == "ShatalovCVDV")
	{
	    if(controllingTemperature != oxygenDissociationHighT and
	       controllingTemperature != oxygenDissociationLowT)
	    {
		   ...
		   (controllingTemperature == dissociation))
		{
		    ...
		    if(this->modifiedTemperature())
		    {
			kf = R.kf(p, modTemperature(colTf), c2)*Z;
			kr = R.kr(kf, p, modTemperature(colTr), c2);
		    }
		    else
		    {
			kf = R.kf(p, colTf, c2)*Z;
			kr = R.kr(kf, p, colTr, c2);
		    }
		}
		else
		...
		/* WarningInFunction
		    << "Temperatures out of bounds for Shatalov's "
		    << "C-V model. Passing to CVDV's one."
		    << endl; */
...
) const
{
	...
	{
	    if(controllingTemperature != oxygenDissociationHighT and
		controllingTemperature != oxygenDissociationLowT and
		controllingTemperature != dissociation)
	    {
		if(this->modifiedTemperature())
		{
	    ...
	    else if(T < Thigh_ and Tv < Thigh_)
	    {
		if((controllingTemperature == oxygenDissociationHighT and T > Tlow_) or
		   (controllingTemperature == oxygenDissociationLowT and T <= Tlow_) or
		   (controllingTemperature == dissociation))
...
Foam::chemistry2Model<CompType, ThermoType>::Scv(label i) const
{
	...
	if(this->thermo().composition().species()[i] == "O2" and 
		(this->CVModel_ == "ShatalovCVDV" or this->CVModel_ == "ShatalovParkTTv"))
	{
	    ...
	    forAll(Scv, cellI)
	    {
		Scv[cellI] = (preferentialFactorShatalov_*dissociationPotential +
		    specieThermo_[i].HEel(pCells[cellI], TvCells[cellI]))*RR_[i][cellI];
	    }

	    /*forAll(Scv, cellI)
	    {
		if (TvCells[cellI] < TtCells[cellI])
		{
		    const scalar vibTemp = specieThermo_[i].vibrationalList()[1];
		    scalar num = (1.0 - exp(-vibTemp/TtCells[cellI]))*TvCells[cellI];
		    scalar den = (1.0 - exp(-vibTemp/TvCells[cellI]))*TtCells[cellI];
		    scalar reciprocalT = 1.0/(1.0/TvCells[cellI] - 1.0/TtCells[cellI]);
		    Scv[cellI] = -2.5*log(num/den)*reciprocalT*RR_[i][cellI];
		}
		else
		{
		    const scalar dissociationPotential = 
        ...
        else if (this->CVModel_ == "ParkTTv" or this->CVModel_ == "ShatalovParkTTv")
        {
