    ...
    
{
    ...
    if (CVModel_ == "ParkTTv" or CVModel_ == "ShatalovParkTTv")
    {
        exponentPark_ = readScalar(subDict("chemistryVibrationCoupling").subDict("ParkTTvCoeffs").lookup("exponentTtr"));
        
        ScvModel_ = subDict("chemistryVibrationCoupling").subDict("ParkTTvCoeffs").lookupOrDefault<word>("sourceTermModel", "preferential");
        ...
        {
            preferentialModel_ = subDict("chemistryVibrationCoupling").subDict("ParkTTvCoeffs")
					.subDict("preferentialModel").lookupOrDefault<word>("factorType", word::null);
    ...
    else if (CVModel_ != "CVDV" and CVModel_ != "ShatalovCVDV" and CVModel_ != "none")
    {
        FatalErrorIn("Foam::basic2ChemistryModel::basic2ChemistryModel(const fvMesh& mesh)")
            << "This chemistryVibrationCoupling model does not exist. Valid models are 'ParkTTv', 'CVDV', 'ShatalovPark' and ShatalovCVDV."
            << exit(FatalError);
    }
}
