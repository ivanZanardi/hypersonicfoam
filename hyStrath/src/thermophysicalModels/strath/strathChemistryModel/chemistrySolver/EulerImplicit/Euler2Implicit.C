...
) const
{
    ...
    simpleMatrix<scalar> RReiiN(nSpecie, 0, 0); // NEW VINCENT 25/03/2016 
    simpleMatrix<scalar> RReiiO(nSpecie, 0, 0); // NEW VINCENT 25/03/2016 
    ...
    const scalar het = mixture.HEt(p, T);
    const scalar hevel = mixture.HEvel(p, Tv);
    scalar deltaTEst = min(deltaT, subDeltaT);

    // Effective reaction rates for N+ and O+
    scalarField rriirN(1); // N+
    scalarField rriirO(1); // O+
    ...
    //Info << "Pt5: Euler2Implicit::solve" << c << endl;
    ceiiN = RReiiN.LUsolve(); // NEW Zanardi
    ceiiO = RReiiO.LUsolve(); // NEW Zanardi
    ...
    for (label i=0; i<nSpecie; i++)
    {
        ...
        ceiiN[i] = max(0.0, ceiiN[i]); // NEW Zanardi
        ceiiO[i] = max(0.0, ceiiO[i]); // NEW Zanardi
    }
    ...
    T = mixture.TtHEt(het, p, T);
    Tv = mixture.TvelHEvel(hevel, p, Tv);
}
// ************************************************************************* //
